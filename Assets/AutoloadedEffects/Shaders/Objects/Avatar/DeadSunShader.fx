...
float4 ApplyHeatCrackEffects(float4 baseColor, float2 sphereCoords, float metalCrack, float highlightBrightness, float specialCrackIntensity)
{
    ...
    float crack = crackNoiseIntensity * 0.7 + distance(sphereCoords, 0.5) * heatCrackEmergePower - metalInfluence;
...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    specialCrackIntensity = pow(specialCrackIntensity, 3.2);
    ...
    // Apply a bottom underglow to the sphere from the Avatar's lily.

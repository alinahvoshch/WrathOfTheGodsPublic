...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    float twinkle = lerp(0.6, 1.4, cos(length(coords - 0.5) * 5.25 + globalTime * 3) * 0.5 + 0.5);
    ...
    float4 baseColor = pow(color, 3) * 3 + smoothstep(0.12, 0.04, distanceFromCenter);
    
    return saturate(baseColor) * twinkle;
}

...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    for (float i = 0; i < 3; i++)
    {
        ...
        // it's relative to the current time, allowing layers to gradually move, giving the illusion that the player is travelling in 3D space in the Avatar's mind.
        float index = Modulo(i - globalTime * forwardScrollSpeed, 3);
        ...
        intensity += reinforcementInterpolant * amplitude * pow(shapeInterpolant * (detailInterpolant + 0.5), 2) * (i >= 1);

...
public class SwordConstellation : ConstellationProjectile, IProjOwnedByBoss<NamelessDeityBoss>
{
    ...
    public Matrix SquishTransformation
    {
        get
        {
            ...
            float squishFactorY = Lerp(1.68f, 1.67f, Projectile.scale);
    ...
    public override int ConvergeTime => NamelessDeityBoss.SwordConstellation_ConstellationConvergeTime;

    public override int StarDrawIncrement => 4;

    public override float StarConvergenceSpeed => 0.00058f;

    public override float StarRandomOffsetFactor => 0.27f;

    public override float StarScaleFactor => base.StarScaleFactor * 1.5f;
    ...
    {
        // Play a blood sound.
        SoundEngine.PlaySound(GennedAssets.Sounds.Common.LargeBloodSpill with { Volume = 3f, PitchVariance = 0.05f }, target.Center);
        ...
        for (int i = 0; i < 15; i++)
        {
            ...
            bloodVelocity.Y -= 12f;
            BloodParticle blood = new BloodParticle(target.Center, bloodVelocity, bloodLifetime, bloodScale, bloodColor);
            blood.Spawn();
        }
        for (int i = 0; i < 25; i++)
        {
            ...
            BloodParticle2 blood = new BloodParticle2(target.Center, bloodVelocity, 30, bloodScale, bloodColor);
            blood.Spawn();
        }
    }

    // Ensure that the blade draws behind Nameless' hand.
    public override void DrawBehind(int index, List<int> behindNPCsAndTiles, List<int> behindNPCs, List<int> behindProjectiles, List<int> overPlayers, List<int> overWiresUI)
    {
        SpecialLayeringSystem.DrawCacheAfterBlack_Proj.Add(index);
    }

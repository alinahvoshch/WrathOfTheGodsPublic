...
public class FireParticleSystem : FastParticleSystem
{
    ...
    protected override void PopulateVertexBufferIndex(VertexPosition2DColorTexture[] vertices, int particleIndex)
    {
        ...
        float scale = InverseLerp(0f, 1f, particle.Time);
        scale += InverseLerp(4f, 24f, particle.Time) * 0.75f;

        float fadeIn = InverseLerp(0f, 5f, particle.Time);
        float glow = 1f + InverseLerp(18f, 0f, particle.Time) * 0.5f;
        ...
        float frameInterpolant = InverseLerp(0f, ParticleLifetime + lifetimeBoost, particle.Time);
        int frameIndex = (int)Lerp(9f, 36f, Pow(frameInterpolant, 0.67f));
        Rectangle frameRectangle = texture.Value.Frame(6, 6, frameIndex % 6, frameIndex / 6);
        Vector2 inverseTextureArea = Vector2.One / texture.Size();

        // Scuffed way of determining whether the alt texture should be used.
        frameInterpolant += particleIndex % 2;

        vertices[vertexIndex] = new VertexPosition2DColorTexture(Unsafe.As<Vector2SIMD, Vector2>(ref topLeftPosition), color, frameRectangle.TopLeft() * inverseTextureArea, frameInterpolant);
        vertices[vertexIndex + 1] = new VertexPosition2DColorTexture(Unsafe.As<Vector2SIMD, Vector2>(ref topRightPosition), color, frameRectangle.TopRight() * inverseTextureArea, frameInterpolant);
        vertices[vertexIndex + 2] = new VertexPosition2DColorTexture(Unsafe.As<Vector2SIMD, Vector2>(ref bottomRightPosition), color, frameRectangle.BottomRight() * inverseTextureArea, frameInterpolant);
        vertices[vertexIndex + 3] = new VertexPosition2DColorTexture(Unsafe.As<Vector2SIMD, Vector2>(ref bottomLeftPosition), color, frameRectangle.BottomLeft() * inverseTextureArea, frameInterpolant);
    }
}

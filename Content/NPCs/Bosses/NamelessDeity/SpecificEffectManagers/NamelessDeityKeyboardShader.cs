...
public class NamelessDeityKeyboardShader : ChromaShader
{
    ...
    public NamelessDeityKeyboardShader()
    {
        ...
        if (Main.netMode != NetmodeID.Server)
        {
            ...
            Texture2D eyeTexture = ModContent.Request<Texture2D>(GetAssetPath("Extra", "NamelessDeityEye"), AssetRequestMode.ImmediateLoad).Value;
    ...
    {
        BrightnessIntensity = Saturate(BrightnessIntensity * 0.96f - 0.005f);

        if (!Main.gamePaused)
        {
            EyeBrightness = Saturate(EyeBrightness * 0.96f - 0.005f);
    ...
    private void ProcessLowDetail(RgbDevice device, Fragment fragment, EffectDetailLevel quality, float time)
    {
        for (int i = 0; i < fragment.Count; i++)
        {
            ...
            if (SeamScale >= 0.001f)
            {
                ...
                gridColor = Vector4.Lerp(gridColor, Vector4.One, Saturate((1f - distanceToLine * brightnessTaper) * SeamScale * 0.33f) * (1f - HeavenlyBackgroundIntensity));
            }

...
public partial class NamelessDeityBoss : ModNPC
{
    private void RemoveBestiaryStarLimits(ILContext context, ManagedILEdit edit)
    {
        ...
        if (starCountField is null)
        {
            edit.LogFailure("The NPCPortraitInfoElement._filledStarsCount field could not be found.");
            return;
        }
    ...
    public override void DrawBehind(int index)
    {
        ...
        if (NPC.hide && canDraw)
        {
            // Special rule: Draw Nameless above tiles at the start of his battle, so that he appears in front of the tree.
            if (CurrentState == NamelessAIType.Awaken || CurrentState == NamelessAIType.OpenScreenTear || CurrentState == NamelessAIType.IntroScreamAnimation)
            {
            ...
            if (CurrentState == NamelessAIType.EndCreditsScene)
                Main.instance.DrawCacheNPCProjectiles.Add(index);
            else if ((DrawCongratulatoryText || UniversalBlackOverlayInterpolant >= 0.02f) && ZPosition >= -0.5f)
    ...
    public override void ModifyTypeName(ref string typeName)
    {
        ...
        if (TestOfResolveSystem.IsActive)
            typeName = Language.GetTextValue($"Mods.{Mod.Name}.NPCs.{Name}.TestOfResolveName");
    }

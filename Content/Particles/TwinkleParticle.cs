    ...
    public static void DrawTwinkle(AtlasTexture texture, Vector2 drawPosition, int totalStarPoints, float rotation, Color backglowBloomColor, Color color, Vector2 scale, float spokesExtendOffset = 0.6f)
    {
        ...
        for (int i = 0; i < instanceCount; i++)
        {
            ...
            for (float s = 1f; s > 0.3f; s -= 0.2f)
                Main.spriteBatch.Draw(texture, drawPosition, null, color, rotation + rotationOffset, null, new Vector2(1f - (spokesExtendOffset - 0.6f) * 0.4f, spokesExtendOffset) * localScale * s, 0);
        }
    }

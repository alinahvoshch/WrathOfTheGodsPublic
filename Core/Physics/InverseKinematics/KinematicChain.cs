...
public class KinematicChain
{
    ...
    /// </summary>
    public const float UpdateStep = 0.24f;
    ...
    private double LossFunction(Vector2 idealEndEffectorPosition, float gradientDescentCompletion) =>
        Pow(EndEffectorPosition.Distance(idealEndEffectorPosition) + (float)joints.Sum(j => j.ConstraintPenalties(gradientDescentCompletion)), 0.1f);
    ...
    private double CalculateLossGradient(Vector2 idealEndEffectorPosition, float gradientDescentCompletion, int inputIndex)
    {
    ...
    public void Update(Vector2 idealEndEffectorPosition)
    {
        ...
        {
            float relaxationFactor = Sqrt(1f - i / stepCount);
            ...
            for (int j = 0; j < JointCount; j++)
            {
                ...
                gradient[j] = (float)SoftClamp(CalculateLossGradient(idealEndEffectorPosition, i / stepCount, j), 0.17);
            }
            ...
            for (int j = 0; j < JointCount; j++)
                joints[j].Rotation = WrapAngle360(joints[j].Rotation - gradient[j] * relaxationFactor * UpdateStep);

            // Update the end effector position, now that the joint configurations have been updated.
            UpdateEndEffector();

            if (EndEffectorPosition.WithinRange(idealEndEffectorPosition, 7.4f))
                break;
        }
    }

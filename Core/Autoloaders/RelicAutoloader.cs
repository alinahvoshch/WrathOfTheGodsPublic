...
{
    [Autoload(false)]
    public class AutoloadableRelicItem(string texturePath) : ModItem
    {
        internal int tileID;

        private readonly string texturePath = texturePath;

        private readonly string name = Path.GetFileName(texturePath).Replace("_Item", "Item");
    ...
    public class AutoloadableRelicTile : ModTile
    {
        ...
        public LazyAsset<Texture2D> RelicTexture;
        ...
        public AutoloadableRelicTile(string texturePath)
        {
            ...
            if (!Main.dedServ)
            {
                ...
                RelicTexture = LazyAsset<Texture2D>.FromPath(texturePath);
            }
        }
    ...
    public static void Create(Mod mod, string texturePathBase, out int itemID, out int tileID)
    {
        // Autoload the item.
        AutoloadableRelicItem relicItem = new AutoloadableRelicItem($"{texturePathBase}_Item");
        mod.AddContent(relicItem);
        itemID = relicItem.Type;

        // Autoload the tile.
        AutoloadableRelicTile relicTile = new AutoloadableRelicTile($"{texturePathBase}_Tile");

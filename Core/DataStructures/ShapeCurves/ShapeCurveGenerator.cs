...
public class ShapeCurveGenerator
{
    internal static void GenerateShapeFile(string imagePath)
    {
        ...
        string shapeFilePath = Path.GetDirectoryName(imagePath) + "\\" + shapeFileName;
        using var shapeFile = File.Create(shapeFilePath);
        using BinaryWriter shapeFileWriter = new using BinaryWriter(shapeFile);
        ...
        Vector2 correction = new Vector2(inputImage.Width, inputImage.Height);

...
public static partial class CommonCalamityVariables
{
    ...
    {
        Calamity?.Code?.GetType("CalamityMod.DownedBossSystem")?.GetField(fieldName, BindingFlags.NonPublic | BindingFlags.Static)?.SetValue(null, value);
    }

    private static void SetWorldValue(string fieldName, bool value)
    {
        Calamity?.Code?.GetType("CalamityMod.World.CalamityWorld")?.GetField(fieldName, BindingFlags.Public | BindingFlags.Static)?.SetValue(null, value);
    }

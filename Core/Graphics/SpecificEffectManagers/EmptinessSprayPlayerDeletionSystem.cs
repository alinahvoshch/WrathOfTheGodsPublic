...
public class EmptinessSprayPlayerDeletionSystem : ModSystem
{
    ...
    {
        orig(self);
        if (PlayerWasDeletedByLaRuga && ModReferences.CalamityRemixMod is not null)
            Main.newMusic = MusicLoader.GetMusicSlot(ModReferences.CalamityRemixMod, "Sounds/Music/LaRuga");
    }
        ...
        if (PlayerWasDeleted)
        {
            ...
            if (DeletionTimer >= 210)
            {
                ...
                ThreadPool.QueueUserWorkItem(new WaitCallback(context =>
                {
                    ...
                    if (PlayerWasDeletedByNamelessDeity)
                        SoundEngine.PlaySound(GennedAssets.Sounds.NamelessDeity.DoNotVocals with { Volume = 1.32f });

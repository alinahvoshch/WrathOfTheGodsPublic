...
namespace NoxusBoss.Core.Graphics.Meshes;
...
public class MeshRegistry : ModSystem
{
    ...
    private static void GenerateMeshes_Cylinder()
    {
        ...
        {
            float angle = TwoPi * i / width;
            for (int j = 0; j <= height; j++)
            {
                Vector2 uv = new Vector2(Cos(angle), j / (float)height);
                Vector3 position = new Vector3(Cos(angle) * 0.5f, uv.Y - 0.5f, Sin(angle) * 0.5f);
                Vector3 normal = new Vector3(Cos(angle), 0f, Sin(angle));
